...
- [ ] Hero: Particle Net canvas animation (sparse gold/white node drift + line draw, 10–20% opacity, continuous loop — brand bible specified, currently missing)
- [ ] Hero text: refine to word-by-word slam-in (scale + Y-translate + opacity per word, 0.6–0.9s total) — brand bible calls for slam not per-char ripple
- [ ] Audit and replace --ease-spring usage in animations (cubic-bezier overshoot is explicitly non-compliant per motion rules — ease-out or precision cubic-bezier only)

### Phase C — Payload admin customisation
- [ ] Custom dashboard root view (welcome message, quick stats: published projects count, last updated)
- [ ] Branded admin theme (dark background matching public site, gold accents)
- [ ] Tidy sidebar (surface only relevant collections: Projects, Site Settings/Globals)
